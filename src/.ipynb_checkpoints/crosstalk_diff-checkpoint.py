...
def computeDiffcomm(comparison, 
    ...
    cond1, cond2 = comparison.split('__vs__')
    ...
    ## === new
    ms_cellpair = original_result[['Metabolite_Name', 'Metabolite', 'Sensor', 'Sender', 'Receiver', 'copresence']]
    ...
    if commu_res.shape[0] > 0:
        ...
        indices = (original_result['copresence'] > 0) & (original_result['metabolite_prop_in_sender'] > prop_cut) & (original_result['sensor_prop_in_receiver'] > prop_cut)
    ...
    ms_cellpair = ms_cellpair.pivot_table(index=['Metabolite_Name', 'Metabolite', 'Sensor', 'Sender', 'Receiver'], 
                                 columns='cond', 
                                 values=['copresence']).reset_index()
    ms_cellpair.columns = ['_'.join(col).strip() if col[1] else col[0] for col in ms_cellpair.columns.values]
    
    ms_cellpair_na = ms_cellpair[pd.isna(ms_cellpair['copresence_'+cond1]) | pd.isna(ms_cellpair['copresence_'+cond2])]
    ms_cellpair = ms_cellpair.dropna()

    ## compute fc
    # mmin = min([ms_cellpair['copresence_'+cond1].min(), ms_cellpair['copresence_'+cond2].min()])
    # ms_cellpair['FC'] = (ms_cellpair['copresence_'+cond1] - mmin) / (ms_cellpair['copresence_'+cond2] - mmin)
    ##  Symmetric Log Fold Change, adding minimal as pseudo to avoid nan and neg
    ep = 1e-06
    amin = abs(min([ms_cellpair['copresence_'+cond1].min(), ms_cellpair['copresence_'+cond2].min()]))+ep
    v1, v2 = (ms_cellpair['copresence_'+cond1]+amin), (ms_cellpair['copresence_'+cond2]+amin)
